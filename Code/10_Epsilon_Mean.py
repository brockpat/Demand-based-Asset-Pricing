...
"""
#%% Prediction Error for zero Holdings
...
#Prepare final results table
Results = pd.DataFrame(columns=['rdate', 'bin', 'Prediction_Error', 'Share_Zero_Assets', 'Restricted'])
...
for quarter in Quarters:
    ...
    #Estimates Sliced
    df_Estimates_Q = df_Estimates[df_Estimates['rdate']==quarter]
    ...
    for i_bin in np.sort(df_Q['bin'].unique()):
        ...
        #Slice Estimates on Bin
        df_Estimates_Q_bin = df_Estimates_Q[df_Estimates_Q['bin'] == i_bin]
        
        #Generate Results Dataframe
        Results_bin = pd.DataFrame(columns=['rdate', 'bin', 'Prediction_Error', 'Share_Zero_Assets','Restricted'])
        Results_bin.at[0,'rdate'] = quarter
        Results_bin.at[0,'bin'] = i_bin
        ...
        #Extract variables
        X = df_Q_bin[['rweight'] + Baseline_endog_Variables_Names + ['constant','cons']]
        share_zeros = len(X[X['rweight'] == 0])/len(X)
        
        #Extract variables for zeros only
        X = X[X['rweight'] == 0]
        y = X['rweight'].values
        X = X.drop('rweight',axis=1).values
        
        #Extract regression coefficients
        beta = df_Estimates_Q_bin[Baseline_endog_Variables_Names + ['constant']].values.reshape(-1)
        
        prediction =  np.exp( X[:,:-1] @ beta + X[:,X.shape[1]-1] )
        
        Results_bin.loc[:,'Prediction_Error'] = np.mean(np.abs(y - prediction))
        Results_bin.loc[:,'Share_Zero_Assets'] = share_zeros
        
        if df_Estimates_Q_bin['LNme'].values == 0.99:
            Results_bin.loc[:,'Restricted'] = True 
           
        else:
            Results_bin.loc[:,'Restricted'] = False 

        Results = pd.concat([Results,Results_bin])

Results.to_csv(path + "/Output" + "/Prediction_Error_Zero_Holdings.csv")

#%% Prediction Error for non-zero Holdings
...
#Prepare final results table
Results = pd.DataFrame(columns=['rdate', 'bin', 'Prediction_Error', 'Share_NonZero_Assets', 'Restricted'])
...
for quarter in Quarters:
    ...
    for i_bin in np.sort(df_Q['bin'].unique()):
        ...
        #Generate Results Dataframe
        Results_bin = pd.DataFrame(columns=['rdate', 'bin', 'Prediction_Error', 'Share_NonZero_Assets', 'Restricted'])
        Results_bin.at[0,'rdate'] = quarter
        Results_bin.at[0,'bin'] = i_bin
        ...
        #Extract variables for non-zeros only
        X = X[X['rweight'] >0]
        y = X['rweight'].values
        X = X.drop('rweight',axis=1).values
        ...
        Results_bin.loc[:,'Prediction_Error'] = np.mean(np.abs(y - prediction))
        Results_bin.loc[:,'Share_NonZero_Assets'] = 1-share_zeros
...
Results.to_csv(path + "/Output" + "/Prediction_Error_NonZero_Holdings.csv")
